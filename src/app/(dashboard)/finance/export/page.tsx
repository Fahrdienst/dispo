...
export default async function FinanceExportPage({ searchParams }: ExportPageProps) {
  ...
  const dateFrom =
    params.dateFrom && isValidDate(params.dateFrom) ? params.dateFrom : getMonthStart()
  const dateTo =
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <Button variant="outline" size="sm" asChild>
          <Link href={`/finance/export?dateFrom=${prev.from}&dateTo=${prev.to}`}>
        ...
        <Button variant="outline" size="sm" asChild>
          <Link href={`/finance/export?dateFrom=${next.from}&dateTo=${next.to}`}>
            Naechster Monat &rarr;
          </Link>
        </Button>
        <Button variant="outline" size="sm" asChild>
          <Link href="/finance/export">Aktueller Monat</Link>
        </Button>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <SummaryCard label="Fahrten" value={summary.totalRides.toString()} />
        <SummaryCard
          label="Gesamtumsatz"
...
function SummaryCard({ label, value, sublabel, variant = "default" }: SummaryCardProps) {
  const borderColor =
    variant === "danger"
      ? "border-red-200"
      : variant === "warning"

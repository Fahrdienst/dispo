...
import { exportStatisticsCsv } from "@/actions/statistics"
import type { StatDimension } from "@/lib/finance/statistics"
 ...
 * emits aggregate columns; patient rows are pseudonymized (SEC-M14-009).
 */
export function StatisticsExportButton({
  dimension,
  dateFrom,
  dateTo,
  disabled = false,
}: StatisticsExportButtonProps): React.ReactElement {
  ...
  function handleExport(): void {
    ...
    startTransition(async () => {
      const result = await exportStatisticsCsv(dimension, dateFrom, dateTo)
      if (!result.success) {

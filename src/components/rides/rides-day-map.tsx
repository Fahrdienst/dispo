...
export async function RidesDayMap({ date }: RidesDayMapProps) {
  ...
  const [patientRes, destinationRes] = await Promise.all([
    ...
    supabase
      .from("rides")
      .select("destinations!inner(id, lat, lng)")
      .eq("date", date)
      .eq("is_active", true)
      .not("status", "in", TERMINAL_STATUSES),
  ])
  ...
  const destCoords = [
    ...new Set(
      (destinationRes.data ?? [])
        .map((r) => {
          const d = r.destinations as unknown as { id: string; lat: number | null; lng: number | null }
